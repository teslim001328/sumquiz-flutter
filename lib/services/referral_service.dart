...
class ReferralService {
  ...
  Future<void> applyReferralCode(String code, String newUserId) async {
    ...
    if (trimmedCode.isEmpty) {
      developer.log('Attempted to apply an empty or whitespace referral code.', name: 'com.example.myapp.ReferralService.applyReferralCode');
      return;
    }

    developer.log('Starting referral process for newUser: $newUserId with code: "$trimmedCode"', name: 'com.example.myapp.ReferralService.applyReferralCode');
    ...
    if (referrerQuery.docs.isEmpty) {
      developer.log('Validation failed: No referrer found for code "$trimmedCode". Aborting.', name: 'com.example.myapp.ReferralService.applyReferralCode');
    ...
    try {
      await _firestore.runTransaction((transaction) async {
        developer.log('Inside transaction. Fetching documents...', name: 'com.example.myapp.ReferralService.transaction');
        ...
        if (referrerDocRef.id == newUserId) {
          developer.log('Validation failed: User $newUserId attempted to refer themselves. Aborting.', name: 'com.example.myapp.ReferralService.transaction');
          return;
        }

        // 3.2: Ensure Referrer Exists
        if (!referrerDoc.exists) {
          developer.log('Validation failed: Referrer document for ID ${referrerDocRef.id} does not exist. Aborting.', name: 'com.example.myapp.ReferralService.transaction');
          return;
        }

        // 3.3: Idempotency Check - Prevent Duplicate Referrals
        // This is the most critical check for preventing reward abuse.
        if (newUserDoc.exists && (newUserDoc.data() as Map<String, dynamic>).containsKey('appliedReferralCode')) {
          final appliedCode = (newUserDoc.data() as Map<String, dynamic>)['appliedReferralCode'];
          developer.log('Validation failed: User $newUserId has already applied a referral code ("$appliedCode"). Aborting.', name: 'com.example.myapp.ReferralService.transaction');
          return;
        }

        developer.log('All validations passed. Proceeding with atomic writes.', name: 'com.example.myapp.ReferralService.transaction');

        // Block 4: Atomic Writes
        // All writes MUST use the `transaction` object.

        // 4.1: Update and Reward the New User (+3 days Pro)
        final newUserData = newUserDoc.exists ? newUserDoc.data() as Map<String, dynamic> : <String, dynamic>{};
        final currentNewUserExpiry = (newUserData['subscriptionExpiry'] as Timestamp?)?.toDate() ?? DateTime.now();
        final newExpiryDateForNewUser = currentNewUserExpiry.add(const Duration(days: 3));
        ...
        developer.log('Prepared write for newUser $newUserId: +3 days Pro, marked as referred by ${referrerDocRef.id}.', name: 'com.example.myapp.ReferralService.transaction');
        ...
        final int currentTotalReferrals = referrerData['totalReferrals'] as int? ?? 0;
        final int currentRewards = referrerData['referralRewards'] as int? ?? 0;
        final DateTime currentReferrerExpiry = (referrerData['subscriptionExpiry'] as Timestamp?)?.toDate() ?? DateTime.now();
        ...
        if (newReferralsCount >= 3) {
          newReferrerExpiryDate = currentReferrerExpiry.add(const Duration(days: 7));
          newRewardsCount += 1;
          newReferralsCount = 0; // Reset for the next cycle.
          developer.log('Referrer ${referrerDocRef.id} hit reward threshold. Granting +7 days and resetting counter.', name: 'com.example.myapp.ReferralService.transaction');
        }
        ...
        developer.log('Prepared write for referrer ${referrerDocRef.id}: referrals and rewards updated.', name: 'com.example.myapp.ReferralService.transaction');
      }); // --- End of transaction ---

      developer.log('Firestore transaction completed successfully for newUser: $newUserId.', name: 'com.example.myapp.ReferralService.applyReferralCode');
    } catch (e, s) {
  ...
  Future<String> generateReferralCode(String uid) async {
    ...
    if (doc.exists && (doc.data() as Map<String, dynamic>).containsKey('referralCode')) {
