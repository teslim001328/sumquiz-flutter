...
class AIService {
  ...
  Future<String> getSuggestion(String text) async {
    ...
    try {
      ...
      developer.log('Error getting suggestion', name: 'my_app.ai_service', error: e);
  ...
  Future<String> generateSummary(String text, {Uint8List? pdfBytes}) async {
    if (pdfBytes != null) {
      ...
      try {
        ...
        throw AIServiceException('Failed to extract text from PDF: ${e.toString()}');
      }
    }
    ...
    try {
      ...
      developer.log('JSON parsing error in summary', name: 'my_app.ai_service', error: e);
      throw AIServiceException('Failed to parse summary data. Please try again.');
    } on TimeoutException {
      throw AIServiceException('Request timed out. Please try again.');
    } catch (e) {
      developer.log('Error generating summary', name: 'my_app.ai_service', error: e);
      throw AIServiceException('Failed to generate summary: ${e.toString()}');
    }
  }

  Future<List<Flashcard>> generateFlashcards(model_summary.Summary summary) async {
    final model = _createModel(AIConfig.textModel);
    final prompt =
    ...
    try {
      ...
      developer.log('JSON parsing error in flashcards', name: 'my_app.ai_service', error: e);
      throw AIServiceException('Failed to parse flashcard data. Please try again.');
    } on TimeoutException {
      throw AIServiceException('Request timed out. Please try again.');
    } catch (e) {
      developer.log('Error generating flashcards', name: 'my_app.ai_service', error: e);
      throw AIServiceException('Failed to generate flashcards: ${e.toString()}');
    }
  }

  Future<Quiz> generateQuizFromText(String text, String title, String userId) async {
    final model = _createModel(AIConfig.textModel);
    final prompt =
    ...
    try {
      ...
      if (response.text == null) {
        throw AIServiceException('Failed to generate quiz: No response from model');
      }
      ...
      developer.log('JSON parsing error in quiz', name: 'my_app.ai_service', error: e);
  ...
  Future<Quiz> generateQuizFromSummary(model_summary.Summary summary) async {
    // This now uses the new generateQuizFromText method to avoid code duplication
  ...
  Future<String> describeImage(Uint8List imageBytes) async {
    ...
    try {
      ...
      developer.log('Error describing image', name: 'my_app.ai_service', error: e);
